...
-- ================================================================
-- gen_auth schema  (auth_service models: user.py, rbac.py)
-- ================================================================
...
-- ================================================================
-- metadata schema  (metadata_service models: metadata.py)
-- ================================================================
...
-- ================================================================
-- query schema  (query_service models: template.py, history.py, analysis.py)
-- ================================================================
...
-- ================================================================
-- connector schema  (connector_service models: connector.py + cheminformatics compound)
-- ================================================================
...
-- ================================================================
-- audit schema  (audit_service models: audit.py)
-- ================================================================
...
-- ================================================================
-- lineage schema  (lineage_service models: lineage.py)
-- ================================================================
...
-- ================================================================
-- bio schema  (bioinformatics_service models: bioinfo.py)
-- ================================================================
...
-- ================================================================
-- workflow schema  (workflow_service models: workflow.py)
-- ================================================================
...
-- ================================================================
-- ai schema  (ai_service models: copilot.py)
-- ================================================================
...
-- ================================================================
-- Migration: Move data from public schema to target schemas
-- ================================================================
-- This block copies data from public.* tables (if they exist) to the
-- correct schema tables. Run this ONCE after creating the schema tables.

DO $$
DECLARE
    tbl RECORD;
BEGIN
    -- Move public.users → gen_auth.users
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='users') THEN
        INSERT INTO gen_auth.users SELECT * FROM public.users ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.users → gen_auth.users';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='roles') THEN
        INSERT INTO gen_auth.roles SELECT * FROM public.roles ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.roles → gen_auth.roles';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='permissions') THEN
        INSERT INTO gen_auth.permissions SELECT * FROM public.permissions ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.permissions → gen_auth.permissions';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='role_permissions') THEN
        INSERT INTO gen_auth.role_permissions SELECT * FROM public.role_permissions ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.role_permissions → gen_auth.role_permissions';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='user_roles') THEN
        INSERT INTO gen_auth.user_roles SELECT * FROM public.user_roles ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.user_roles → gen_auth.user_roles';
    END IF;

    -- Move public metadata tables → metadata.*
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='metadata_fields') THEN
        INSERT INTO metadata.metadata_fields SELECT * FROM public.metadata_fields ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.metadata_fields → metadata.metadata_fields';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='metadata_entities') THEN
        INSERT INTO metadata.metadata_entities SELECT * FROM public.metadata_entities ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.metadata_entities → metadata.metadata_entities';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='metadata_values') THEN
        INSERT INTO metadata.metadata_values SELECT * FROM public.metadata_values ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.metadata_values → metadata.metadata_values';
    END IF;

    -- Move public query tables → query.*
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='query_templates') THEN
        INSERT INTO query.query_templates SELECT * FROM public.query_templates ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.query_templates → query.query_templates';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='query_history') THEN
        INSERT INTO query.query_history SELECT * FROM public.query_history ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.query_history → query.query_history';
    END IF;

    -- Move public connector tables → connector.*
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='data_sources') THEN
        INSERT INTO connector.data_sources SELECT * FROM public.data_sources ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.data_sources → connector.data_sources';
    END IF;

    -- Move public audit tables → audit.*
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='audit_logs') THEN
        INSERT INTO audit.audit_logs SELECT * FROM public.audit_logs ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.audit_logs → audit.audit_logs';
    END IF;

    -- Move public workflow tables → workflow.*
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='workflow_definitions') THEN
        INSERT INTO workflow.workflow_definitions SELECT * FROM public.workflow_definitions ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.workflow_definitions → workflow.workflow_definitions';
    END IF;

    -- Move public AI tables → ai.*
    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='chat_sessions') THEN
        INSERT INTO ai.chat_sessions SELECT * FROM public.chat_sessions ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.chat_sessions → ai.chat_sessions';
    END IF;

    IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname='public' AND tablename='chat_messages') THEN
        INSERT INTO ai.chat_messages SELECT * FROM public.chat_messages ON CONFLICT DO NOTHING;
        RAISE NOTICE 'Migrated public.chat_messages → ai.chat_messages';
    END IF;
END$$;

-- ================================================================
-- Done. All schemas, tables, and indexes are created.
-- Data from public.* has been migrated to the correct schemas.
-- ================================================================
